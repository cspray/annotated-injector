...
class InjectMultipleProfilesSecondMethodParamTest extends AnnotatedTargetDefinitionConverterTestCase {
    ...
    protected function getSubjectTarget(): AnnotatedTarget {
        return $this->getAnnotatedTarget(AttributeType::Inject, new \ReflectionParameter(
            [Fixtures::injectConstructorServices()->injectProfilesStringService()->name(), '__construct'],
            'val'
        ), 1);
    }
    ...
    public function testGetValue() {
        $this->assertSame('from-test', $this->definition->value());
    }
    ...
    public function testGetProfiles() {
        $this->assertSame(['test'], $this->definition->profiles());
    }

    public function testGetAttribute() {
        self::assertInstanceOf(Inject::class, $this->definition->attribute());
        self::assertSame('from-test', $this->definition->attribute()->value());
    }
}
